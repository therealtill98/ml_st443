...
\paragraph{Decision.} In selecting our “best” model, we considered performance, interpretability, efficiency, and alignment with the underlying data structure, ultimately choosing \emph{Logistic Regression (PCA 10)}. Although \emph{SVM (RBF, PCA 10)} attains marginally higher scores (by roughly 0.02–0.23 percentage points), these improvements come with substantially higher computational cost, greater sensitivity to hyperparameters, and an increased risk of overfitting due to the model’s non-linear flexibility. Given that the data are already close to linearly separable after scaling and PCA compression, the expressive power of the RBF kernel offers limited practical benefit and increases the likelihood of modelling noise rather than structure.

In contrast, Logistic Regression provides a more conservative inductive bias against overfitting, benefiting from its linear decision boundary and stable optimization. Its coefficients remain directly interpretable in PCA space, clarifying how latent spectral components contribute to class predictions, and its computational footprint is small relative to the examined non-linear models. We therefore regard \emph{Logistic Regression (PCA 10)} as the most balanced and operationally suitable option, maintaining near-optimal performance (\(\approx 0.9894\) accuracy, \(\approx 0.9878\) macro F1, \(\approx 0.9999\) AUC) while remaining efficient, interpretable, and robust to overfitting.
